...
for r in data['rows']:
    fv=r.get('fair_value')
    out.append({'ticker':r['ticker'],'as_of':data['as_of'],'status':'ready' if fv else 'blocked','decision':'invest' if fv else 'abstain','fair_value_available':bool(fv),'valuation':fv,'financial_periods':r.get('financial_periods',5 if r.get('line_items') else 0),'blockers':[] if fv else [r.get('warning') or 'No multi-model fair value'],'next_actions':[] if fv else ['Acquire/validate missing financial or price evidence'],'company_name':r.get('company_name'),'sector':r.get('sector'),'source':r.get('source'),'diagnostics':r.get('diagnostics',{}),'earnings_quality':r.get('earnings_quality',{}),'event_status':r.get('event_status','unknown'),'event_flags':r.get('event_flags',[])})

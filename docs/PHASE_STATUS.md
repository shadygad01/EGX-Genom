...
## Investment Science Lab / Claim Registry (2026-08-02)

**DONE (architecture and operating surface).** The existing versioned
`MethodologyRegistry` now owns a Claim Registry extending its literature-source
catalog into the hypothesis methodology pipeline. Claims independently track
hypothesis, experiment, validation, shadow-fund, investment-proof, and terminal
promotion/rejection states. Production knowledge generated by the daily pipeline
has claim provenance; attaching a paper never changes eligibility. The registry
is exported as `claims.json` and rendered in the bilingual Research screen.
Shadow-fund and investment-proof stages stay pending until real claim-specific
...
The Investment Science Lab follow-up now reads the registry's owned claim
store and turns explicit paper claims into independently versioned lifecycle
entities. Literature collection itself remains planned; no paper source is
treated as operational decision data. New `test_methodology_registry.py` (5

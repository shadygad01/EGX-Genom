...
def seed_sources() -> list[SourceSpec]:
    return [
        ...
        # Primary financial-document routes. The generic collector is implemented and
        # tested; activation still requires a verified per-company URL, so blocked or
        # unresolved constituents remain visible rather than silently guessed.
        _spec(
            id="egx_financial_statements",
            name="EGX official financial statements and disclosures",
            category=SourceCategory.OFFICIAL,
            country="EG",
            access_method=AccessMethod.HTML_SCRAPE,
            status=SourceStatus.IMPLEMENTED,
            legal_use_status=LegalUseStatus.RESEARCH_ONLY,
            base_url="https://www.egx.com.eg/en/newslist.aspx?ID=15",
            reliability_score=0.95,
            freshness_score=0.8,
            conflict_priority=99,
            ...
            license="Official EGX public disclosure pages; preserve attribution and verify redistribution terms.",
            terms_of_use_url="https://www.egx.com.eg/en/Disclaimer.aspx",
            supported_event_types=["corporate"],
            supported_languages=["ar", "en"],
            notes="Follows only real report links found on the official page; PDFs are archived but withheld until a source-specific parser validates their layout.",
        ),
        _spec(
            id="fra_financial_statements",
            name="FRA official issuer disclosures",
            category=SourceCategory.OFFICIAL,
            country="EG",
            access_method=AccessMethod.HTML_SCRAPE,
            status=SourceStatus.IMPLEMENTED,
            legal_use_status=LegalUseStatus.RESEARCH_ONLY,
            base_url="https://fra.gov.eg/en/",
            reliability_score=0.95,
            freshness_score=0.75,
            conflict_priority=98,
            collector="OfficialFilingsCollector",
            ...
            rate_limit=RateLimit(requests_per_minute=4, min_seconds_between_requests=3.0),
            license="Official FRA public pages; preserve attribution and verify redistribution terms.",
            terms_of_use_url="https://fra.gov.eg/en/",
            supported_event_types=["corporate"],
            supported_languages=["ar", "en"],
            notes="The connector degrades visibly when FRA blocks automated access; it never bypasses access controls.",
        ),
        _spec(
            id="mubasher_financials",
            name="Mubasher EGX financial statements",
            category=SourceCategory.MARKET_DATA,
            country="EG",
            access_method=AccessMethod.HTML_SCRAPE,
            status=SourceStatus.IMPLEMENTED,
            ...
            conflict_priority=60,
            collector="MubasherFinancialsCollector",
            collector_version="1.0.0",
            retry_policy=RetryPolicy(max_attempts=2, backoff_seconds=1.0, backoff_multiplier=2.0),
            rate_limit=RateLimit(requests_per_minute=20, min_seconds_between_requests=0.3),
            license="Public Mubasher financial-statement pages; secondary source, preserve attribution and verify terms before redistribution.",
            terms_of_use_url="https://english.mubasher.info/",
            validation_rules=["structured financialStatement object required", "null values skipped"],
            normalization_rules=["Mubasher period label mapped to annual/quarterly", "currency preserved from page object"],
            supported_event_types=["corporate"],
            supported_languages=["en", "ar"],
            notes="Secondary fallback for EGX statements; use only when primary and StockAnalysis sources do not produce usable line items.",
        ),
        _spec(
            id="stockanalysis_financials",
            name="StockAnalysis EGX financial statements tables",
            category=SourceCategory.MARKET_DATA,
            country="EG",
            access_method=AccessMethod.HTML_SCRAPE,
            status=SourceStatus.IMPLEMENTED,
            ...
            reliability_score=0.7,
            freshness_score=0.7,
            conflict_priority=65,
            collector="StockAnalysisFinancialsCollector",
            collector_version="1.0.0",
            retry_policy=RetryPolicy(max_attempts=2, backoff_seconds=1.0, backoff_multiplier=2.0),
            rate_limit=RateLimit(requests_per_minute=30, min_seconds_between_requests=0.2),
            license="Public StockAnalysis pages; secondary source, preserve attribution and verify terms before redistribution.",
            terms_of_use_url="https://stockanalysis.com/terms-of-service/",
            validation_rules=["annual period and visible value required", "no TTM/current/growth rows materialized"],
            normalization_rules=["EGX ticker URL -> uppercase ticker", "visible annual fiscal period retained", "financial statement source labeled secondary"],
            supported_event_types=["corporate"],
            supported_languages=["en"],
            notes="Secondary fallback for broad EGX coverage; primary EGX/FRA/issuer filings override when available and period-aligned.",
        ),
        _spec(
            id="company_ir",
            name="Company Investor Relations financial reports",
            category=SourceCategory.COMPANY,
            access_method=AccessMethod.PDF_DOWNLOAD,
            status=SourceStatus.IMPLEMENTED,
            legal_use_status=LegalUseStatus.RESEARCH_ONLY,
            ...
            collector_version="1.0.0",
            retry_policy=RetryPolicy(max_attempts=1),
            rate_limit=RateLimit(requests_per_minute=10, min_seconds_between_requests=1.0),
            supported_event_types=["corporate"],
            supported_languages=["ar", "en"],
            notes="Generic IR discovery follows one level of real report links from verified issuer homepages; no domain is fabricated.",
        ),

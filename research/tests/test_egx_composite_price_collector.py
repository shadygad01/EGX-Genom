...
def yahoo_payload(ticker="COMI", latest="2026-07-26"):
    ...
    return json.dumps({
        "chart": {"result": [{
            ...
            "timestamp": [timestamp],
            "indicators": {"quote": [{
                "open": [70.0], "high": [72.0], "low": [69.0],
                "close": [71.5], "volume": [123456],
            }]},
            "events": {
                "dividends": {str(timestamp): {"date": timestamp, "amount": 1.25}},
                "splits": {str(timestamp - 1): {
                    "date": timestamp, "numerator": 2, "denominator": 1,
                    "splitRatio": "2:1",
                }},
            },
        }], "error": None},
...
def test_parse_yahoo_price_and_corporate_actions():
    ...
    collector = EgxCompositePriceCollector(
        price_spec(), [ticker], fetcher=fetcher,
...
def test_per_ticker_fallback_reaches_mubasher_after_close():
    ...
    fetcher = MappingFetcher({
        ...
        stock_urls[0]: KeyError("not found"), stock_urls[1]: KeyError("not found"),
    ...
    assert (bar.ticker, bar.trade_date, bar.close, bar.volume) == (
        "ICFC", date(2026, 7, 26), 15.47, 517127,
    )
...
def test_universe_symbols_are_injected_without_a_fixed_ticker_list():
    ...
    collector = build_live_collector(
        "egx_price_composite", price_spec(), fetcher=fetcher, tickers=["AAA", "BBB"],
    )
...
def test_live_wiring_scopes_owner_authorized_robots_override_to_price_collector():
    ...
    collector = build_live_collector(
        "egx_price_composite", price_spec(), fetcher=shared, tickers=["COMI"],
    )
...
def test_collected_prices_reach_market_memory_and_decision_readiness(tmp_path: Path):
    ...
    for offset in range(20):
        ...
        rows.append(
            f'{{a:10,c:10,h:11,l:9,o:10,t:"{day.isoformat()}",v:100000,ch:0}}'
        )
    ...
    collector = EgxCompositePriceCollector(
        price_spec(), [ticker],
        fetcher=MappingFetcher({
            yahoo_url: KeyError("not listed"), stock_url: stock_payload,
        }),
        now=lambda: datetime(2026, 7, 26, 15, tzinfo=CAIRO),
    )
    ...
    state = MarketMemory(
        LocalCsvDataProvider(tmp_path), universe, StaticSectorProvider({}), lookback_days=30,
    ).reconstruct(as_of)

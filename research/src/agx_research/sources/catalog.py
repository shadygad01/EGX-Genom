...
def seed_sources() -> list[SourceSpec]:
    return [
        ...
        _spec(
            ...
            normalization_rules=["dates ISO-8601", "OHLCV floats/ints", "symbol upper-cased, .eg stripped"],
        ...
        _spec(
            ...
            access_method=AccessMethod.CSV_DOWNLOAD, status=SourceStatus.PLANNED,
            reliability_score=0.9, freshness_score=0.8,
            historical_coverage="official listings/indices/disclosures",
            update_frequency="daily", conflict_priority=95,
        ...
        _spec(
            ...
            access_method=AccessMethod.CSV_DOWNLOAD, status=SourceStatus.PLANNED,
            reliability_score=0.95, freshness_score=0.8, conflict_priority=98,
            supported_entities=["EGP rates", "policy rates", "inflation"],
            supported_event_types=["macroeconomic"], supported_languages=["ar", "en"],
            notes="Publishes exchange/policy rates and time series; endpoint verification pending.",
        ),
        _spec(
            id="mof_egypt", name="Ministry of Finance (Egypt)", category=SourceCategory.OFFICIAL,
            access_method=AccessMethod.PDF_DOWNLOAD, status=SourceStatus.PLANNED,
            reliability_score=0.9, freshness_score=0.5, conflict_priority=90,
            supported_event_types=["macroeconomic"], supported_languages=["ar", "en"],
        ),
        _spec(
            id="capmas", name="CAPMAS", category=SourceCategory.OFFICIAL,
            access_method=AccessMethod.PDF_DOWNLOAD, status=SourceStatus.PLANNED,
            reliability_score=0.9, freshness_score=0.4, conflict_priority=90,
            supported_event_types=["macroeconomic"], supported_languages=["ar"],
        ),
        _spec(
            id="egypt_open_data", name="Government Open Data (Egypt)", category=SourceCategory.OFFICIAL,
            access_method=AccessMethod.JSON_API, status=SourceStatus.PLANNED,
            reliability_score=0.85, freshness_score=0.4, conflict_priority=85,
            supported_event_types=["macroeconomic"], supported_languages=["ar", "en"],
        ),
        # ---- COMPANY (PLANNED; per-company IR config) ----
        _spec(
            id="company_ir", name="Company Investor Relations (per-constituent config)",
            category=SourceCategory.COMPANY, access_method=AccessMethod.PDF_DOWNLOAD,
            status=SourceStatus.PLANNED, reliability_score=0.85, freshness_score=0.5,
            conflict_priority=90,
            supported_event_types=["corporate"], supported_languages=["ar", "en"],
        ...
        _spec(
            id="yahoo_finance", name="Yahoo Finance", category=SourceCategory.MARKET_DATA,
            access_method=AccessMethod.JSON_API, status=SourceStatus.TOS_REVIEW,
            reliability_score=0.7, freshness_score=0.9, conflict_priority=55,
        ...
        _spec(
            ...
            access_method=AccessMethod.JSON_API, status=SourceStatus.NEEDS_KEY,
            reliability_score=0.7, freshness_score=0.8, conflict_priority=55,
            authentication="api_key(user-supplied)",
        ),
        _spec(
            id="tiingo", name="Tiingo (free tier)", category=SourceCategory.MARKET_DATA,
            access_method=AccessMethod.JSON_API, status=SourceStatus.NEEDS_KEY,
            reliability_score=0.7, freshness_score=0.8, conflict_priority=55,
            authentication="api_key(user-supplied)",
        ),
        _spec(
            id="investing_com", name="Investing.com", category=SourceCategory.MARKET_DATA,
            access_method=AccessMethod.HTML_SCRAPE, status=SourceStatus.TOS_REVIEW,
            reliability_score=0.6, freshness_score=0.8, conflict_priority=40,
            notes="ToS restricts automated collection; blocked.",
        ),
        _spec(
            id="tradingview", name="TradingView (incl. news)", category=SourceCategory.MARKET_DATA,
            access_method=AccessMethod.JSON_API, status=SourceStatus.TOS_REVIEW,
        ...
        *[
            _spec(
                id=source_id, name=name, category=SourceCategory.NEWS,
                access_method=AccessMethod.RSS_FEED, status=SourceStatus.PLANNED,
                reliability_score=rel, freshness_score=0.9,
                collector="RssNewsCollector", conflict_priority=priority,
                supported_event_types=["news"],
        ...
        *[
            _spec(
                ...
                collector="RssNewsCollector", conflict_priority=40,
        ...
        *[
            _spec(
                id=source_id, name=name, category=SourceCategory.MACROECONOMIC,
                access_method=access, status=SourceStatus.PLANNED,
                reliability_score=0.9, freshness_score=0.5, conflict_priority=85,
                supported_event_types=["macroeconomic"],
                notes=note,
            )
        ...
        _spec(
            id="global_benchmarks", name="Global benchmarks via Stooq/FRED series",
            category=SourceCategory.GLOBAL_MARKETS, access_method=AccessMethod.CSV_DOWNLOAD,
            status=SourceStatus.IMPLEMENTED, collector="StooqPriceCollector/FredCsvCollector",
            collector_version="1.0.0",
            reliability_score=0.75, freshness_score=0.7, conflict_priority=60,
            supported_entities=[
                "S&P500", "NASDAQ", "Dow Jones", "FTSE", "US Treasury yields",
                "Dollar Index", "Gold", "Silver", "Oil (Brent/WTI)", "Natural Gas", "Copper",
            ],
        ...
        _spec(
            id="suez_canal_stats", name="Suez Canal Authority statistics",
            category=SourceCategory.GLOBAL_MARKETS, access_method=AccessMethod.PDF_DOWNLOAD,
            status=SourceStatus.PLANNED, reliability_score=0.9, freshness_score=0.3,
            conflict_priority=90, supported_event_types=["macroeconomic"],
            supported_languages=["ar", "en"],
        ),
        # ---- ALTERNATIVE ----
        *[
            _spec(
                id=source_id, name=name, category=SourceCategory.ALTERNATIVE,
                access_method=access, status=status,
                reliability_score=0.35, freshness_score=0.8, conflict_priority=20,
                notes=note,
            )
            for source_id, name, access, status, note in [
                ("wikipedia_pageviews", "Wikipedia Page Views", AccessMethod.JSON_API,
                 SourceStatus.PLANNED, "Official free Wikimedia API; collector pending."),
                ("google_trends", "Google Trends", AccessMethod.JSON_API,
                 SourceStatus.TOS_REVIEW, "No official API; automation ToS-ambiguous; blocked."),
                ("github_releases", "GitHub Releases (listed-company tech signals)",
                 AccessMethod.JSON_API, SourceStatus.PLANNED, "Official free API."),
                ("company_social_official", "Official company social accounts (LinkedIn/FB/X/YouTube)",
                 AccessMethod.RSS_FEED, SourceStatus.TOS_REVIEW,
                 "Platform APIs require keys and restrict automation; per-platform review needed."),
                ("public_telegram", "Public Telegram channels", AccessMethod.JSON_API,
                 SourceStatus.TOS_REVIEW, "Bot API needs a user bot token; channel-content licensing varies."),
                ("patents", "Patent databases", AccessMethod.JSON_API, SourceStatus.PLANNED,
                 "EPO/WIPO free APIs; Egypt-relevant mapping pending."),
                ("hiring_signals", "Hiring signals (official career pages/feeds)",
                 AccessMethod.RSS_FEED, SourceStatus.PLANNED, "Only official postings; no scraping of third-party job boards without ToS review."),
            ]
        ],
        # ---- RESEARCH ----
        *[
            _spec(
                id=source_id, name=name, category=SourceCategory.RESEARCH,
                access_method=access, status=status,
                reliability_score=0.7, freshness_score=0.3, conflict_priority=30,
                notes=note,
            )
            for source_id, name, access, status, note in [
                ("arxiv", "arXiv (q-fin)", AccessMethod.RSS_FEED, SourceStatus.PLANNED,
                 "Official free API/RSS."),
                ("ssrn", "SSRN", AccessMethod.RSS_FEED, SourceStatus.PLANNED, "Public RSS for new papers."),
                ("nber", "NBER", AccessMethod.RSS_FEED, SourceStatus.PLANNED, "Public new-papers feed."),
                ("google_scholar", "Google Scholar", AccessMethod.HTML_SCRAPE,
                 SourceStatus.TOS_REVIEW, "Explicitly prohibits automated access; blocked."),
                ("researchgate", "ResearchGate", AccessMethod.HTML_SCRAPE,
                 SourceStatus.TOS_REVIEW, "ToS restricts automated collection; blocked."),
            ]
        ],
    ]
